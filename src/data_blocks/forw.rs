...
impl FORW {
    pub fn new(bytes: &[u8]) -> Self {
        ...
        let intermediate_chrg_cause = IntermediateChrgCause::new(&bytes[155..157]).value; // IntermediateChrgCause::new(&bytes[155..157]).value;
        let orig_dialling_class = OrigDiallingClass::new(&bytes[157..159]).value; //"not implemented".to_string(); //    W(  1)       157
                                                                                  // let leg_call_reference = LegCallReference::new(&bytes[159..164]).value; // TODO                  //    C(  5)       159
        let leg_call_reference = "not implemented".to_string();
        let call_reference_time = CallReferenceTime::new(&bytes[164..171]).value;
        let speech_version = SpeechVersion::new(bytes[171]).value; //    C(  1)       171
        ...
        let tns_carrier_code = TNSCarrierCode::new(&bytes[227..229]).value;
        let carrier_selection = CarrierSelection::new(bytes[229]).value;
        let pic = PIC::new(&bytes[230..232]).value;
        let routing_category = RoutingCategory::new(bytes[232]).value;
        let ms_classmark3 = MSClassMark3::new(bytes[233]).value;
        let forwarding_cell_band = CellBand::new(bytes[234]).value;
        let camel_call_reference = CamelCallReference::new(&bytes[235..243]).value;
        let camel_exchange_id_ton = TON::new(bytes[243]).value;
        let camel_exchange_id = CamelExchangeId::new(&bytes[244..253]).value;
        let npdb_query_status = NPDBQueryStatus::new(bytes[253]).value;
        let scp_connection = SCPConnection::new(bytes[254]).value;
        let loc_routing_number_ton = TON::new(bytes[255]).value;
        let loc_routing_number = NUMBER::new(&bytes[256..268]).value;
        let forwarding_msrn_ton = TON::new(bytes[268]).value;
        let forwarding_msrn = MSRN::new(&bytes[269..281]).value;
        let optimal_routing_indicator = OptimalRoutingIndicator::new(bytes[281]).value;
        let add_routing_category = AddRoutingCategory::new(&bytes[282..284]).value;
        let in_bnc_connection_type = BncConnectionType::new(bytes[284]).value;
        let inside_user_plane_index = UserPlaneIndex::new(&bytes[285..287]).value;
        let inside_control_plane_index = ControlPlaneIndex::new(&bytes[287..289]).value;
        let out_bnc_connection_type = BncConnectionType::new(bytes[289]).value;
        let outside_user_plane_index = UserPlaneIndex::new(&bytes[290..292]).value;
        let outside_control_plane_index = ControlPlaneIndex::new(&bytes[292..294]).value;
        // let collect_call_indicator = CollectCallIndicator::new(&bytes[294..295]).value;
        let collect_call_indicator = "not implemented".to_string();
        let forwarding_first_ci = CI::new(&bytes[295..297]).value;
        let forwarding_last_ex_id_ton = TON::new(bytes[297]).value;
